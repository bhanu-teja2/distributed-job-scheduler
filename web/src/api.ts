...
/** Error returned when the API envelope or HTTP status indicates failure. */
export class ApiError extends Error{constructor(public code:string,message:string,public status:number){super(message)}}
/** Development credentials are session-scoped and disappear when the tab closes. */
...
/** Calls the scheduler API and unwraps its stable response envelope. */
